...
var class_name =
{
    <?php echo "0:''"; foreach ($character_class as $id => $class)
    {
        echo(", ".$id.":'".$class."'");
    }
    ?>
    ...
    while (i < data.length)
    {
        ...
        if(n == point_count)
        {
            ...
            if(in_array(data[i].map, maps_array))
            {
                ...
                mpoints[point_count].single_text = data[i].zone+'<br\>等级'+data[i].level+'<br\>'+char+'&nbsp;<img src=\'<?php echo $img_base2 ?>'+data[i].cl+'.gif\' style=\'float:center\' border=0 width=18 height=18\><br\>'+race_name[data[i].race]+'<br/>'+class_name[data[i].cl]+'<br/>';
